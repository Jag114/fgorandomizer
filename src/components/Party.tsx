...
import servantFetch from "../data/servantFetch.ts";
...
const Party = () => {
  ...
  const checkIfDuplicate = (data, multi) => {
    ...
    const savedProfile = [
      ...JSON.parse(localStorage.getItem(`userProfile-${region}`)).map((e) =>
        parseInt(e)
      ),
    ]; //ids are saved as strings

    const usedIDArr = servantList.map((s) => {
      //ids used in party on screen
      return s.id;
    });
    let availableIDArr = data.data.map((e) => {
      //ids from all servants currently available
      return e.collectionNo;
    });
    if (savedProfile.length > 0) {
      availableIDArr = availableIDArr.filter((e) => {
        //ids from all servants currently available - servants not included in users profile
        return savedProfile.includes(e);
      });
    }
    let filteredAvailableIDArr = availableIDArr.filter((id) => {
      //ids from all servants currently available and in users profile - ids used in party on screen
    ...
    while (arrID.length < 5) {
      ...
      if (multi === false) {
        ...
        if (
          usedIDArr.includes(filteredAvailableIDArr[i].collectionNo) === false
        ) {
          data.data.forEach((e) => {
  ...
  const handleClick = (multi, number) => {
    servantFetch(formData, region).then((data) => {
      if (multi === true && data.length < 5) {
        return alert(
          "Too few servants, need more than 5, " + data.length + " chosen now"
        );
      } else if (data.length < 1) {
        return alert(
          "No servants to choose from, check region and filter settings"
        );
      }
  ...
  const servantsDisplay = servantList.map((e, i) => (
    <Servant key={i} number={i} handleClick={handleClick} servantInfo={e} />
  ));
  ...
  return (
    ...
    <div className="party-container">
      ...
      <div className="party-container-regions">
        <ToggleButton text="NA" type="double" active={true}/>
        <ToggleButton text="JP" type="double" active={false}/>
      </div>

      <div className="party-container-filters">
        <ToggleButton text="Filter" type="single" active={false}/>
      </div>

      {servantData.map((servant, i) => {
        return (
          <div className="party-container-servant">
            <Servant key={i} data={servant} />
            <RandomizeButton text="1 x Randomize" type="single"/>
          </div>
        );
      })}

      <div className="party-container-multi">
        <RandomizeButton text="5 x Randomize" type="multi"/>
      </div>
